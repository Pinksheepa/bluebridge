...
{
    if(strcmp(rx_uart,"X") == 0)
    {
        sprintf(tx_uart,"X:%d",x_freq);
        HAL_UART_Transmit(&huart1,(uint8_t *)tx_uart,strlen(tx_uart),50);
    }
    if(strcmp(rx_uart,"Y") == 0)
    {
        sprintf(tx_uart,"Y:%d",y_volt);
        HAL_UART_Transmit(&huart1,(uint8_t *)tx_uart,strlen(tx_uart),50);
    }

    if(strcmp(rx_uart,"PA4") == 0)
    {
        sprintf(tx_uart,"PA4:%.2f",volt_pa4);
        HAL_UART_Transmit(&huart1,(uint8_t *)tx_uart,strlen(tx_uart),50);
    }
    if(strcmp(rx_uart,"PA5") == 0)
    {
        sprintf(tx_uart,"PA5:%.2f",volt_pa5);
        HAL_UART_Transmit(&huart1,(uint8_t *)tx_uart,strlen(tx_uart),50);
    }
    if(strcmp(rx_uart,"PA1") == 0)
    {
        sprintf(tx_uart,"PA1:%.f",freq_pa1);
        HAL_UART_Transmit(&huart1,(uint8_t *)tx_uart,strlen(tx_uart),50);
    }
    ...
    if(strcmp(rx_uart,"#") == 0)
    {
        ...
        {
            LCD_Clear(Black);
            LCD_Init_overturn();   // set SS and SM bit		  //0x0100
            
        }
            else {LCD_Clear(Black); LCD_Init();}
    }    
